...
const scenes = [
  ...
  {from: 1920, duration: 480, kind: 'proof' as const},
  {from: 2400, duration: 300, kind: 'close' as const},
];

const subtitles = [
  [0, 3.7, 'A protocol pause key should not live with one person.'],
  [3.7, 7.2, 'Holdfast makes containment a private two-person decision.'],
  [7.2, 12.8, 'A detector watches one Solana vault and reconstructs the prior balance.'],
  [12.8, 17.8, 'This planted withdrawal is 25 percent. The policy triggers above 20.'],
  [17.8, 22.2, 'The detector can open an incident. It cannot pause the vault.'],
  [22.2, 27.7, 'The incident moves into a permissioned MagicBlock Ephemeral Rollup.'],
  [27.7, 32.4, 'Individual approvals stay private. One responder is never enough.'],
  [32.4, 37.4, 'Two distinct responders must approve the same stored action hash.'],
  [37.4, 41.8, 'Quorum does not authorize an arbitrary runbook.'],
  [41.8, 47.5, 'It commits one expiring pause action against one separate vault program.'],
  [47.5, 51.5, 'Magic Actions carry that decision back to Solana.'],
  [51.5, 54.5, 'A commit is not containment.'],
  [54.5, 60.4, 'If the pause fails, Holdfast keeps the receipt pending and says unpaused.'],
  [60.4, 65.6, 'Success requires an executed receipt and the matching paused vault.'],
  [65.6, 70.1, 'Judges can verify the finished run without a wallet.'],
  [70.1, 76.1, 'The browser reads both deployed accounts directly from MagicBlock Devnet.'],
  [76.1, 80.7, 'Their owners, layouts, status, and incident identifiers must all agree.'],
  [80.7, 85.0, 'Holdfast turns protocol response into a constrained, inspectable decision.'],
  [85.0, 86.7, 'Two people can stop the test drain. One person cannot.'],
] as const;
